...
const separator="@@@";
const VERSION=1;
export function asKey(mult, expr, params) {
    return [VERSION, mult, expr, ...params].join(separator);
}
export function parseKey(key) {
    const  [version, mult, expr, ...params] = key.split(separator);
    return {mult, expr, params};
}
